...
class Frekterbitan extends MY_Controller {
	...
	public function __construct(){
      ...
      $this->load->model($this->class.'_model','m_data');
    }
	  ...
	  public function edit($id=null){
	      if ($this->input->is_ajax_request()) {      
	        $data = $this->m_data->get_it(['id'=>$id],'id, nama, keterangan');        
		...
		protected function stat_error(){
			$errors = array(
				...
				'input_keterangan'     =>  form_error('input_keterangan'),
			);
			return $errors;
		}
    ...
    protected function data_post($mode){
    	...
    	$data = array(
				...
				'keterangan'  => html_escape($this->input->post('input_keterangan')),
    	);
			if ($mode=='edit') {
			}else{
			}        
    ...
    protected function is_related($id){
    	$decision=false;
 			$arr = array(
 					 		$this->m_data->get_relation_data('jurnal','frek_terbitan_id',$id)
 					 	);
		...
		protected function validate($mode=null){
	        ...
	        $this->form_validation->set_rules('input_nama'  ,'Nama' ,'trim|max_length[50]|required');
	        $this->form_validation->set_rules('input_keterangan'  ,'Keterangan' ,'trim|max_length[2000]|required');
		}  
