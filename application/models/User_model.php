...
class User_model extends MY_Model {
    ...
    $this->load->database();
		...
		$this->sql_view  = ' (SELECT u.id, u.username, u.name, u.email, u.telepon, u.provinsi_id ,p.name as provinsi_nama, u.institusi, u.is_verified, u.is_active, u.last_login ';
		$this->sql_view .= ' FROM user u ';
		$this->sql_view .= ' LEFT JOIN provinsi p ON p.id = u.provinsi_id) AS sql_view';
	}

	public function query_data(){
		$sql  = ' SELECT id, username, name, email, telepon, provinsi_id, provinsi_nama, institusi, is_verified, is_active, last_login ';
